...
parser.add_argument('--profile-fx-node-time', type=bool, default=False, help='"True": profile by FX nodes (case B). Warning: skips DDP wrapping.')
...
profile_fx_node_time = args.profile_fx_node_time
...
optimus_p = Optimus_p(model, gas, 
                      ...
                      # Module-level profiling (works with DP/DDP): group ops by transformer block.
                      profile_layer_time=profile_cut, profile_layer_time_steps=profile_step,
                      # (B) FX node profiling: measure per-FX-node compute and group by transformer block.
                      profile_fx_node_time=profile_fx_node_time, profile_fx_node_time_steps=profile_step)
...
def train():
    ...
    for i, batch in enumerate(dataloader):
        ...
        if profile_cut and profile_step <= i:
            break
