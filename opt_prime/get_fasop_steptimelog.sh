...
INPUT_CSV="${2:-/workspace/FASOP/main_logs/llama_evenly_mbs_steptime.csv}"
EXP="fasop_steptime_logs"
LOG_DIR="./_logs"
OUT_CSV="${3:-${LOG_DIR}/${DATE}_${EXP}.csv}"
mkdir -p "$LOG_DIR"
...
if [[ "${#CONFIG_LINES[@]}" -eq 0 ]]; then
  echo "[error] no configs found in $INPUT_CSV" >&2
  exit 2
fi

echo "[info] extracted ${#CONFIG_LINES[@]} unique configs"
...
for line in "${CONFIG_LINES[@]}"; do
  ...
  for i in $(seq 1 "$TRYNUMBER"); do
    ...
    ./get_steptime_avg.sh "${log_prefix}_log_llama4_${i}.txt" > "${log_prefix}_steptime_log_llama4_${i}.txt" 2>&1 || {
      echo "[warn] get_steptime_avg.sh failed (try=${i}) for $config_name"
