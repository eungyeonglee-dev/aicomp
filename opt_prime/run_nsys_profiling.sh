# CONFIGS: GBS MBS TP DP PP
docker exec optimus-timelog bash -lc \
'cd /workspace/aicomp/opt_prime \
&& CONFIGS=("1 1 1 1 8" "2 1 1 2 4"); \
for cfg in "${CONFIGS[@]}"; do read GBS MBS TP DP PP <<< "$cfg"; \
EXT="_MBS${MBS}_PP${PP}_TP${TP}_DP${DP}_GBS${GBS}"; 
LOG_DIR=./_logs; NSYS_DIR=./_nsys_log; DATE=$(date +%Y%m%d_%H%M%S); mkdir -p $LOG_DIR; \
LOG_FILE=${LOG_DIR}/${DATE}${EXT}_nsys_log.txt; \
NSYS_OUT=${NSYS_DIR}/${DATE}${EXT}_nsys; \
...
--profile-cut True --profile-step 3 --profile-start-step 50 > ${LOG_FILE} 2>&1; \
echo "nsys finished"; ls -1 ${NSYS_OUT}.nsys-rep ${NSYS_OUT}.qdstrm 2>/dev/null || true; \
nsys stats --report nvtxsum ${NSYS_OUT}.nsys-rep | head -n 60 || true; tail -n 40 ${LOG_FILE}; done'