...
# sudo docker exec -it $CONTAINER_NAME \
 ...
 sudo docker exec -it $CONTAINER_NAME \
                /bin/bash -lc 'cd /workspace/aicomp/opt_prime/opt_prime/tutoruslabs && LOGFILE=./results/$(date +%Y%m%d%H%M%S).log; GPULOGFILE=./results/$(date +%Y%m%d%H%M%S)_gpustats.log; MEMLOGFILE=./results/$(date +%Y%m%d%H%M%S)_memstats.log; (while true; do echo "===== $(date "+%F %T") ====="; gpustat --no-color || true; echo; sleep 1; done) >> "$GPULOGFILE" 2>&1 & GPUSTAT_PID=$!; (while true; do echo "===== $(date "+%F %T") ====="; free -h || true; echo; sleep 1; done) >> "$MEMLOGFILE" 2>&1 & MEMSTAT_PID=$!; trap "kill $GPUSTAT_PID $MEMSTAT_PID 2>/dev/null || true; wait $GPUSTAT_PID $MEMSTAT_PID 2>/dev/null || true" EXIT INT TERM; bash ./run_rdzv_70b.sh $1 $2 $3 $4 $5 $6 > "$LOGFILE" 2>&1;'                
